...
async def api_create_error_log_csv(
    ...
    try:
        ...
        if result_data.get("status") == "error":
            raise HTTPException(
                status_code=500, 
                detail=result_data.get("message", "An error occurred while generating the CSV.")
            )
        
        csv_content = result_data.get("csv_content")
        file_name = result_data.get("file_name", "error_log.csv")

        if not csv_content:
             raise HTTPException(status_code=500, detail="Tool did not generate CSV content.")
        ...
        print(f"ERROR in api_create_error_log_csv: {e}")
